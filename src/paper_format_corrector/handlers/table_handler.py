...
class TableHandler:
    ...
    def _format_table(self, table):
        ...
        # 设置表格宽度（如果配置了）
        # self._set_table_width(table, Cm(14.5))  # 14.5cm 适合 A4

        for row_idx, row in enumerate(table.rows):
            for col_idx, cell in enumerate(row.cells):
                self._format_cell(cell, row_idx, col_idx, len(table.rows), len(table.columns))

    def _format_cell(self, cell, row_idx, col_idx, total_rows, total_cols):
        """格式化单个单元格"""
        for para in cell.paragraphs:
            self._format_cell_paragraph(para)

    def _format_cell_paragraph(self, paragraph):
        """格式化单元格内段落"""
        # 设置字体
