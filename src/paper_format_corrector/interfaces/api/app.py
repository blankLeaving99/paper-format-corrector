...
from ...infrastructure.queue.worker import Worker
...
async def generate_correction_plan(
    ...
    with tempfile.TemporaryDirectory() as tmp_dir:
        ...
        try:
            ...
            from ..application.style_workbench import plan_to_dict
    ...
    with tempfile.TemporaryDirectory() as tmp_dir:
        ...
        try:
            if ext == "pdf":
                # PDF 反向学习
                try:
                    from ..domain.document.pdf_style_extractor import PDFStyleExtractor
...
def validate_template_config(config: dict):
    """验证模板配置的完整性和正确性。"""
    from ..application.template_validation_service import TemplateValidationService
...
def start_university_import(university: str, requirement_file: str):
    ...
    """
    from ..application.university_template_import_service import UniversityTemplateImportService
...
async def export_latex(file: UploadFile = File(...)):
    ...
    with tempfile.TemporaryDirectory() as tmp_dir:
        ...
        try:
            from ..infrastructure.converters.latex_exporter import LaTeXExporter
