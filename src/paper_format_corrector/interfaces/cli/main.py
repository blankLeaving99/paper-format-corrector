...
from ...infrastructure.doc_template_loader import list_doc_templates, load_doc_template
from ...infrastructure.preset_loader import format_preset_list, get_preset_choices
...
def cmd_scan(args: argparse.Namespace) -> None:
    """扫描文档结构和样式"""
    from .application.style_workbench import build_correction_plan, scan_document
...
def cmd_learn(args: argparse.Namespace) -> None:
    """从样本文档学习格式"""
    from .application.style_workbench import explain_style_profile, learn_style_profile
...
def _template_update(repo, args):
    ...
    from .infrastructure.updater import VersionChecker
    ...
    from .infrastructure.updater import AutoUpdater
...
def cmd_report(args: argparse.Namespace) -> None:
    """生成或查看报告"""
    from .application.report_service import ReportData, ReportService
...
def _handle_list_models(args) -> None:
    """列出当前provider下所有可用模型"""
    from .domain.document.model_discovery import list_models
...
def _handle_probe_model(args) -> None:
    """探测指定模型是否可用"""
    from .domain.document.model_discovery import probe_model
...
def cmd_database(args: argparse.Namespace) -> None:
    """数据库管理子命令"""
    from paper_format_corrector.infrastructure.database import (
...
def _cmd_correct(args: argparse.Namespace) -> None:
    ...
    if language and language != "auto":
        from ...application.style_workbench import get_language_font_config
...
def _run_flat_mode(args: argparse.Namespace) -> None:
    ...
    if args.flat_file:
        if args.flat_output:
            from .infrastructure.path_security import ALLOWED_OUTPUT_EXTENSIONS, validate_output_path
