...
def process_paper(paper_file, requirement_file, config_file, template_file, preset_name, export_formats, do_score, do_diff):  # noqa: C901
    ...
    # 校验论文文件扩展名
    from .adapters.path_security import ALLOWED_INPUT_EXTENSIONS
...
def preview_correction_plan(
    ...
    if paper_file is None:
        return "请先上传 .docx 论文。"
    try:
        from .services.style_workbench import build_correction_plan
...
def template_check_update():
    ...
    try:
        from .adapters.updater import AutoUpdater, VersionChecker
...
def check_rules(paper_file, rules_file):
    ...
    c = PaperFormatCorrector(config_path)
    from .adapters.path_security import validate_input_path
