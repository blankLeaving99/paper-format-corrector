...
from ...adapters.storage.doc_template_loader import list_doc_templates
...
class FuzzyCombobox(ttk.Combobox):
    ...
    用户输入时自动过滤下拉选项，支持中英文模糊匹配。
    - 输入“deep”会显示“DeepSeek (深度求索)”
    - 输入“深度”也会显示“DeepSeek (深度求索)”
    - 支持拼音首字母搜索（如“sd”匹配“深度”）
    """

    def __init__(self, parent, values=None, textvariable=None, width=20,
                 state="readonly", **kwargs):
        # 保存原始全量值列表
        self._all_values = list(values or [])
        super().__init__(
            parent, values=self._all_values,
    ...
    def _on_key_release(self, event):
        ...
        if event.keysym in (
            ...
            "Alt_L", "Alt_R", "Home", "End", "Prior", "Next",
        ):
            return
        ...
        # 如果没匹配到，尝试拼音首字母匹配
        if not filtered and self._has_pypinyin():
            filtered = self._fuzzy_pinyin_match(typed)

        self.config(values=filtered if filtered else self._all_values)
    ...
    def _fuzzy_pinyin_match(self, typed):
        """拼音首字母模糊匹配"""
        try:
            ...
            for v in self._all_values:
                # 获取拼音首字母
                initials = lazy_pinyin(v, style=Style.FIRST_LETTER)
                initials_str = "".join(initials).lower()
                if typed in initials_str:
                    results.append(v)
            return results
        except Exception:
            return []
...
class PaperFormatDesktopApp:
    ...
    def __init__(self):
        ...
        self.wb_language = tk.StringVar(value="auto")
        ...
        self.gen_provider = tk.StringVar(value="deepseek")
        self.gen_key = tk.StringVar()
        self.gen_model = tk.StringVar()
        self.gen_base_url = tk.StringVar()
        self.gen_model_list = []  # 已获取的模型列表
        self.gen_provider.trace_add("write", self._on_provider_changed)
    ...
    def _build_workbench_tab(self, notebook):
        ...
        self.wb_template_box = FuzzyCombobox(template_row, textvariable=self.wb_template_choice, values=self._workbench_template_choices(), width=55, state="readonly")
        self.wb_template_box.pack(side=tk.LEFT, padx=5, fill=tk.X, expand=True)

        # ── 语言选择与字体预览 ──
        lang_frame = ttk.LabelFrame(tab, text="语言与字体", padding=10)
        lang_frame.pack(fill=tk.X, padx=10, pady=5)
        lang_row = ttk.Frame(lang_frame)
        lang_row.pack(fill=tk.X, pady=2)
        ttk.Label(lang_row, text="文档语言:", width=12).pack(side=tk.LEFT)
        self.wb_lang_combo = FuzzyCombobox(
            lang_row, textvariable=self.wb_language,
            values=["auto", "chinese", "english", "japanese", "korean"],
            width=20, state="readonly",
        )
        self.wb_lang_combo.pack(side=tk.LEFT, padx=5)
        ttk.Label(lang_row, text="(自动检测 | 中文 | 英文 | 日文 | 韩文)", foreground="gray").pack(side=tk.LEFT, padx=5)
        self.wb_language.trace_add("write", self._on_language_change)
        self.wb_font_preview = ttk.Label(lang_frame, text="", foreground="#333", wraplength=600)
        self.wb_font_preview.pack(fill=tk.X, pady=(5, 0))
        ...
        FuzzyCombobox(styles, textvariable=self.wb_table_style, values=["three_line", "full_border", "keep"], width=12, state="readonly").grid(row=3, column=1, sticky=tk.W)
        ttk.Label(styles, text="图片最大宽度").grid(row=3, column=2, sticky=tk.W, padx=4, pady=3)
        FuzzyCombobox(styles, textvariable=self.wb_image_width, values=["full", "90%", "80%", "70%", "50%"], width=12, state="readonly").grid(row=3, column=3, sticky=tk.W)
        ...
        type_combo = FuzzyCombobox(override_row, textvariable=self.override_type_var,
    ...
    def _on_language_change(self, *_args):
        """Update font fields when language selection changes."""
        lang = self.wb_language.get()
        if lang == "auto":
            return
        cfg = get_language_font_config(lang)
        if cfg["body_font"]:
            self.wb_body_font.set(cfg["body_font"])
        if cfg["heading_font"]:
            self.wb_heading_font.set(cfg["heading_font"])
        self._update_font_preview()

    def _update_font_preview(self):
        """Update the font preview label based on current language."""
        lang = self.wb_language.get()
        if lang == "auto":
            text = "语言: 自动检测 — 将根据文档内容自动选择字体"
        else:
            cfg = get_language_font_config(lang)
            preset = LANGUAGE_FONT_PRESETS.get(lang, {})
            text = (
                f"语言: {preset.get('label', lang)}  |  "
                f"正文: {cfg['body_font']}  |  "
                f"标题: {cfg['heading_font']}  |  "
                f"英文: {cfg['en_font']}"
            )
        self.wb_font_preview.config(text=text)
    ...
    def _build_template_tab(self, notebook):
        ...
        self.tm_category_combo = FuzzyCombobox(filter_row1, textvariable=self.tm_category_var,
        ...
        self.tm_source_combo = FuzzyCombobox(filter_row1, textvariable=self.tm_source_var,
        ...
        self.tm_trust_combo = FuzzyCombobox(filter_row2, textvariable=self.tm_trust_var,
...
{json_preview}""")
    ...
    def _open_template_form(self, title, slug=None):
        ...
        for label, key in fields:
            ...
            vars_dict[key] = var
            if key == "category":
                FuzzyCombobox(f, textvariable=var, values=["高校毕业论文", "国际期刊与会议", "引用与写作规范", "个人", "导入模板"],
                             state="readonly", width=30).pack(side=tk.LEFT, fill=tk.X, expand=True)
            elif key == "language":
                FuzzyCombobox(f, textvariable=var, values=["中文", "英文", "日文", "韩文"],
                             state="readonly", width=30).pack(side=tk.LEFT, fill=tk.X, expand=True)
            else:
    ...
    def _validate_template(self):
        ...
        if stored is None:
            ...
            return
        from .services.template_validation import TemplateValidationService
    ...
    def _import_template_dialog(self):
        ...
        FuzzyCombobox(dialog, textvariable=cat_var,
    ...
    def _check_template_updates(self):
        ...
        def do_check():
            try:
                from paper_format_corrector.adapters.updater import AutoUpdater, VersionChecker
                repo = self._get_tm_repo()
    ...
    def _build_ai_gen_tab(self, notebook):
        ...
        # 提供商选择
        from paper_format_corrector.adapters.llm.provider_config import PROVIDERS
        provider_names = [p.display_name for p in PROVIDERS.values()]
        row1 = ttk.Frame(llm_frame)
        row1.pack(fill=tk.X, pady=2)
        ttk.Label(row1, text="提供商:", width=8).pack(side=tk.LEFT)
        self.gen_provider_combo = FuzzyCombobox(
            row1, values=provider_names, width=18, state="readonly",
        )
        self.gen_provider_combo.pack(side=tk.LEFT, padx=5)
        self.gen_provider_combo.bind("<<ComboboxSelected>>", self._on_provider_combo_selected)
        # 设置初始值
        self._update_provider_combo()

        # API Key
        row2 = ttk.Frame(llm_frame)
        row2.pack(fill=tk.X, pady=2)
        ttk.Label(row2, text="API Key:", width=8).pack(side=tk.LEFT)
        ttk.Entry(row2, textvariable=self.gen_key, width=20, show="*").pack(side=tk.LEFT, padx=5, fill=tk.X, expand=True)

        # Base URL (可编辑，用于自定义提供商)
        row_url = ttk.Frame(llm_frame)
        row_url.pack(fill=tk.X, pady=2)
        ttk.Label(row_url, text="API地址:", width=8).pack(side=tk.LEFT)
        ttk.Entry(row_url, textvariable=self.gen_base_url, width=20).pack(side=tk.LEFT, padx=5, fill=tk.X, expand=True)

        # 模型选择 (Combobox + 获取按钮)
        row3 = ttk.Frame(llm_frame)
        row3.pack(fill=tk.X, pady=2)
        ttk.Label(row3, text="模型:", width=8).pack(side=tk.LEFT)
        self.gen_model_combo = FuzzyCombobox(row3, textvariable=self.gen_model, width=18, state="normal")
        self.gen_model_combo.pack(side=tk.LEFT, padx=5, fill=tk.X, expand=True)

        # 获取模型按钮
        row_btn = ttk.Frame(llm_frame)
        row_btn.pack(fill=tk.X, pady=4)
        self.fetch_models_btn = ttk.Button(
            row_btn, text="获取可用模型",
            command=self._fetch_available_models,
        )
        self.fetch_models_btn.pack(side=tk.LEFT, padx=5)
        ttk.Button(
            row_btn, text="测试连接",
        ...
        FuzzyCombobox(
            row4, textvariable=self.gen_doc_type, width=18,
            values=["通用文档", "报告", "公文", "合同", "论文", "方案", "会议纪要", "可行性报告", "项目立项", "工作总结", "调研报告"],
        ).pack(side=tk.LEFT, padx=5)
        ...
        FuzzyCombobox(row5, textvariable=self.gen_template, values=doc_templates, width=18, state="normal").pack(side=tk.LEFT, padx=5)
        ...
        tips = """1. 选择 AI 提供商（推荐 DeepSeek，性价比高）

2. 填入 API Key，点击“获取可用模型”自动加载
...
7. 生成完成后点击“导出docx”下载"""
    ...
    def _update_provider_combo(self):
        """更新提供商下拉框"""
        from paper_format_corrector.adapters.llm.provider_config import PROVIDERS, get_provider_by_name
        provider_names = [p.display_name for p in PROVIDERS.values()]
        self.gen_provider_combo.set_values(provider_names)
        # 设置当前值
        current_name = self.gen_provider.get()
        config = get_provider_by_name(current_name)
        self.gen_provider_combo.set(config.display_name)

    def _on_provider_combo_selected(self, event=None):
        """提供商下拉框选择时的回调"""
        from paper_format_corrector.adapters.llm.provider_config import get_provider_by_display
        display_name = self.gen_provider_combo.get()
        config = get_provider_by_display(display_name)
        self.gen_provider.set(config.name)

    def _on_provider_changed(self, *args):
        """提供商 StringVar 变化时的回调"""
        from paper_format_corrector.adapters.llm.provider_config import get_provider_by_name
        provider_name = self.gen_provider.get()
        config = get_provider_by_name(provider_name)
        # 自动填充默认 base_url
        self.gen_base_url.set(config.default_base_url)
        # 自动填充默认模型
        self.gen_model.set(config.default_model)
        # 更新模型下拉框的候选列表
        self.gen_model_combo.set_values(config.popular_models)
    ...
    def _fetch_available_models(self):
        ...
        provider_name = self.gen_provider.get()
        config = get_provider_by_name(provider_name)
        api_key = self.gen_key.get().strip()
        base_url = self.gen_base_url.get().strip() or config.default_base_url

        if config.requires_api_key and not api_key:
            messagebox.showwarning("提示", f"请先填写 {config.display_name} 的 API Key")
            return

        self.fetch_models_btn.config(state=tk.DISABLED)
        self.ai_status_label.config(text=f"正在获取 {config.display_name} 可用模型...")

        def do_work():
            try:
                from paper_format_corrector.core.document.model_discovery import list_models
                models = list_models(config.name, api_key, base_url)
                if models:
                    self.root.after(0, lambda: self._update_model_list(models, config))
                else:
                    self.root.after(0, lambda: self._on_fetch_failed(config))
            except Exception as e:
                self.root.after(0, lambda: self._on_fetch_error(e))

        threading.Thread(target=do_work, daemon=True).start()

    def _update_model_list(self, models, config):
        """更新模型下拉列表"""
        self.gen_model_list = models
        self.gen_model_combo.set_values(models)
        if models and not self.gen_model.get():
            self.gen_model.set(models[0])
        self.ai_status_label.config(
            text=f"已获取 {len(models)} 个可用模型"
        )
        self.fetch_models_btn.config(state=tk.NORMAL)

    def _on_fetch_failed(self, config):
        """获取模型失败"""
        # 回退到预设模型列表
        self.gen_model_combo.set_values(config.popular_models)
        self.ai_status_label.config(
            text=f"API获取失败，已加载预设模型列表"
        )
        self.fetch_models_btn.config(state=tk.NORMAL)

    def _on_fetch_error(self, error):
        """获取模型异常"""
        self.ai_status_label.config(text=f"获取失败: {error}")
        self.fetch_models_btn.config(state=tk.NORMAL)
    ...
    def _test_ai_connection(self):
        ...
        api_key = self.gen_key.get().strip()
        base_url = self.gen_base_url.get().strip() or config.default_base_url
        model = self.gen_model.get().strip() or config.default_model
        ...
        self.ai_status_label.config(text=f"正在测试 {model}...")

        def do_work():
            try:
                from paper_format_corrector.core.document.model_discovery import probe_model
                result = probe_model(config.name, model, api_key, base_url)
                if result.get("available"):
                    latency = result.get("latency_ms", 0)
                    self.root.after(0, lambda: self.ai_status_label.config(
                        text=f"连接成功! 延迟: {latency:.0f}ms"
                    ))
...
{CONTACT_INFO}"""
    ...
    def _run_batch_correct(self):
        ...
        def do_work():
            try:
                ...
                if tpl:
                    from paper_format_corrector.adapters.path_security import validate_input_path
    ...
    def _process_single(self, paper):  # noqa: C901
        """处理单个文件，返回结果文本"""
        from paper_format_corrector.adapters.path_security import ALLOWED_INPUT_EXTENSIONS
        from paper_format_corrector.adapters.path_security import validate_input_path as _vip
    ...
    def _get_ai_generator(self):
        ...
        if self._ai_session is None:
            # 将显示名称转换为提供商 key
            display_name = self.gen_provider.get()
            config = get_provider_by_display(display_name)
