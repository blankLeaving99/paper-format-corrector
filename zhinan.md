...
| ------------ | ------------------------------------------------------------------------------------- |
| CLI 命令行   | 已有                                                                                  |
| Web GUI      | 已有 Gradio 界面                                                                      |
| 桌面 GUI     | 已有 tkinter 界面                                                                     |
| 预设模板     | 已有 IEEE、Nature、Science、APA、Chinese Thesis、ACM、Elsevier、Springer 等 YAML 预设 |
| 文档矫正核心 | 已有`PaperFormatCorrector` 和 `FormatCorrector`                                   |
| 需求解析     | 已有规则解析和 LLM 解析基础                                                           |
| 文档分析     | 已有基础分析器                                                                        |
| 表格处理     | 已有表格处理器，支持进一步扩展三线表                                                  |
| 图片处理     | 已有图片居中和尺寸处理基础                                                            |
| 差异报告     | 已有 HTML 差异报告基础                                                                |
| 质量评分     | 已有评分模块                                                                          |
| 模板库       | 已初步加入 SQLite 模板库                                                              |
| 格式工作台   | 已初步加入扫描、样本学习、手动统一样式                                                |

### 2.2 需要继续补齐的核心短板

| 短板                       | 影响                                         |
| -------------------------- | -------------------------------------------- |
| 文档结构识别还不够细       | 标题、正文、图题、表题、代码块、公式可能误判 |
| 样本文档学习还不够完整     | 只能学习部分字体、段落、表格、图片规则       |
| 模板库还缺完整管理能力     | 需要导入、编辑、版本、分类、来源、启停、导出 |
| 官方高校模板未形成可信流程 | 不能随便写死学校规则，需要来源验证           |
| 修改报告还不够细           | 用户需要知道改了什么、没改什么、为什么没改   |
| GUI 交互还偏工具化         | 需要更接近“格式工作台”的专业体验           |
| 自动化测试覆盖仍需扩大     | 模板、样本学习、批量处理、GUI 逻辑都要测     |
...
| 服务                            | 职责                                   |
| ------------------------------- | -------------------------------------- |
| `StyleWorkbenchService`       | 扫描、手动规则生成、样本学习、应用报告 |
| `TemplateLibraryService`      | 模板查询、导入、编辑、版本管理         |
| `CorrectionPlanService`       | 生成矫正计划和影响范围                 |
| `BatchCorrectionService`      | 批量处理                               |
| `ReportService`               | 生成 Markdown、HTML、JSON 报告         |
| `TemplateValidationService`   | 校验模板规则是否完整可用               |
| `RequirementIngestionService` | 从 Word/PDF/Markdown 要求中提取规则    |

### 5.3 推荐统一数据结构

| 数据结构             | 用途                     |
| -------------------- | ------------------------ |
| `StyleProfile`     | 样本文档学习出的样式画像 |
| `FormatRuleSet`    | 可执行格式规则           |
| `CorrectionPlan`   | 执行前的修改计划         |
| `CorrectionResult` | 执行后的修改结果         |
| `SkippedItem`      | 未修改项和原因           |
| `TemplateRecord`   | 数据库模板记录           |
| `ValidationReport` | 模板或文档校验报告       |
...
| 表格三线表           | 支持论文常用三线表                    | 增强`table_handler.py`                                |
| 图片批量统一         | 统一图片宽度、居中、题注              | 增强`image_handler.py` 和图题处理                     |
| 测试覆盖             | 保证核心功能不回归                    | 新增 style workbench、template repository、handler 测试 |

### P1：强烈建议完成

| 任务               | 目标                             | 实现路径                     |
| ------------------ | -------------------------------- | ---------------------------- |
| 模板管理界面       | 用户能查看、编辑、导入、导出模板 | 新增模板库 Tab               |
| 模板版本管理       | 高校/期刊要求可按年份保存        | 扩展 SQLite schema           |
| 需求文档导入模板   | 上传学校要求后生成模板           | 复用 requirement parser      |
| 低置信度人工修正   | 用户能纠正识别错误               | 工作台加入元素类型编辑       |
| 修改前计划预览     | 执行前知道影响范围               | 增加 dry-run correction plan |
| 参考文献一致性检查 | 检查正文引用和文末列表           | 增强 cross reference         |
| 批量处理报告       | 多文件处理后有汇总               | 新增 batch service           |
| 报告中心           | 历史报告可查看和下载             | 入库处理记录和报告摘要       |

### P2：增强体验

| 任务                 | 目标                      | 实现路径                       |
| -------------------- | ------------------------- | ------------------------------ |
| 官方高校模板导入流程 | 可维护全国高校模板        | 来源文件导入、人工复核、版本化 |
| 国际期刊模板扩展     | 支持更多投稿格式          | 从官方模板和通用规范整理规则   |
| 图片清晰度检查       | 提醒图片过小或拉伸过度    | 读取图片像素和展示尺寸         |
| 公式编号和保护       | 避免公式被误改，支持编号  | 公式检测和专用 handler         |
| 代码块保护           | 代码不被当正文乱改        | 识别等宽字体和代码模式         |
| 多语言字体规则       | 中英日韩字体分别处理      | run 级字体策略                 |
| 导出报告格式         | HTML、Markdown、JSON、PDF | ReportService 统一生成         |

### P3：长期拓展

| 任务             | 目标                   | 实现路径                   |
| ---------------- | ---------------------- | -------------------------- |
| 本地 HTTP API    | 让外部系统调用         | FastAPI 或轻量本地服务     |
...
| 测试         | 覆盖内容                         |
| ------------ | -------------------------------- |
| 模板库测试   | 新建、查询、更新、分类、版本     |
| 样本学习测试 | 正文、标题、表格、图片规则提取   |
| 手动规则测试 | UI 输入转`format_rules`        |
| 表格测试     | 三线表、全框线、表头、多表格     |
| 图片测试     | 宽度、居中、比例保持             |
