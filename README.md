...
- **格式工作台与模板库** — 扫描正文、标题、表格、图片；可学习已排版论文并保存为本地 SQLite 模板，后续一键复用
...
在 Web GUI 或桌面 GUI 的「格式工作台」中可以：

1. 上传 `.docx` 论文，先扫描标题、正文、表格、图片等元素；
2. 选择内置的期刊/毕业论文模板，或上传一份排版正确的样本文档；
3. 统一设置正文、三级标题、表格边框和图片宽度，再应用到所有同类元素；
4. 将样本文档学习到的规则保存为「我的模板」，以后直接从模板库复用；
5. 查看覆盖报告和 HTML 差异报告，复核未自动修改的代码段、公式和未识别元素。

模板库使用本地 SQLite 数据库 `data/template_library.db`，不需要网络服务。内置 YAML 预设会在首次使用时自动写入模板库；个人模板只保存在本机。学校规范应以学校发布的最新要求或可靠样本文档为来源，再保存为个人/高校模板。
